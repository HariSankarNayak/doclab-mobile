...
export class AppointmentViewPage implements OnInit {
  ...
  getData(id: number) {
    ...
    this.provideSvc.postData('appointment-view.php', postData).subscribe(
      (data) => {
        if (data != null) {
          this.items = data;
          console.log(data);
          ...
          this.doctorName =
            data[0].doctor_lastname + ' ' + data[0].doctor_firstname;
          ...
          this.imgURL =
            this.provideSvc.imgURL +
            this.clinicID +
            '/doctor/' +
            this.doctorAvatar;

          const futureDate = new Date(this.appDate);
  ...
  confirm() {
    ...
    this.provideSvc.postData('appointment-confirm.php', dataPost).subscribe(
      (res) => {
  ...
  cancel() {
    ...
    this.provideSvc.postData('appointment-cancel.php', dataPost).subscribe(
      (res) => {
        this.getData(this.appID); // Success
      },
      (error) => {
