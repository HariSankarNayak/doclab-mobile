...
export class ResultPage implements OnInit {
  ...
  loadData() {
    this.providerSvc.getData('result_clinic.php').subscribe(
      (data) => {
        if (data != null) {
    ...
    this.providerSvc.getData('result_doctor.php').subscribe(
      (data) => {
        if (data != null) {
  ...
  filterItems(searchTerm) {
    this.itemsClinic.filter((item) => item.title.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1);
  }
  setFilteredItems() {
