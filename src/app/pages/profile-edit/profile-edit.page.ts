    ...
    if (
      ...
      if (
        ...
        if (this.formData.email.match(this.ctrl.pattern.email)) {
          if (
            !isNaN(this.formData.contact) &&
            this.formData.contact.length < 14
          ) {
            ...
            this.providerSvc.postData('profile-edit.php', dataPost).subscribe(
              (res) => {
                this.ctrl.alertPopUp('Successful', 'Updated', 'OK');
                this.storage.set('USER_INFO_DATA', res).then((data) => {
                  this.getData();
                });
              },
              (error) => {
            ...
            this.ctrl.alertPopUp(
              'Attention',
              'Contact Number only number and not exceed 13!',
              'Try Again'
            );
          }
        } else {
        ...
        this.ctrl.alertPopUp(
          'Attention',
          'First/Lastname Only Text are Allowed!',
          'Try Again'
        );
      }
    } else {
