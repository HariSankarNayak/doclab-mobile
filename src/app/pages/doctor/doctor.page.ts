...
export class DoctorPage implements OnInit {
    ...
    this.storage.get('USER_INFO_DATA').then(
      (data) => {
        if (data != null) {
          ...
          this.patientName =
            data[0].patient_firstname + data[0].patient_lastname;
        }
      },
      (error) => {
        console.log(error);
  ...
  getData(id: number) {
    ...
    this.providerSvc.postData('doctor_profile.php', postData).subscribe(
      (data) => {
        if (data != null) {
          this.items = data;
          console.log(data);
  ...
  getRating() {
    ...
    this.providerSvc.postData('review-details.php', postData).subscribe(
      (data) => {
        if (data != null) {
  ...
  dateSlotChange($event) {
    ...
    this.providerSvc.postData('available.php', dataPost).subscribe(
      (data) => {
